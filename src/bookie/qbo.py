...
_log = logging.getLogger("bookie.qbo")
...
def _api_call(cfg: QBOConfig, creds_path: Path, method: str, path: str,
    ...
    try:
        with urllib.request.urlopen(req, timeout=60) as r:
            ...
            _log.info("QBO %s %s -> HTTP %s intuit_tid=%s request_id=%s",
                      method, path, r.status, tid, rid)
            return json.loads(r.read().decode() or "{}")
    except urllib.error.HTTPError as e:
        # Always capture Intuit's transaction id (intuit_tid) so support can trace failures.
        tid = e.headers.get("intuit_tid") if e.headers else None
        if e.code == 401:
            # Force refresh and retry once
            _log.warning("QBO %s %s -> 401 (refreshing) intuit_tid=%s", method, path, tid)
            cfg.access_token = ""
            cfg = _ensure_access_token(cfg, creds_path)
            ...
            try:
                with urllib.request.urlopen(req2, timeout=60) as r:
                    tid = r.headers.get("intuit_tid")
                    _log.info("QBO %s %s -> HTTP %s intuit_tid=%s request_id=%s (post-refresh)",
                              method, path, r.status, tid, rid)
                    return json.loads(r.read().decode() or "{}")
            except urllib.error.HTTPError as e2:
                tid2 = e2.headers.get("intuit_tid") if e2.headers else None
                _log.error("QBO %s %s failed after refresh: HTTP %s intuit_tid=%s",
                           method, path, e2.code, tid2)
                raise QBOError(f"QBO {method} {path} failed after refresh: HTTP {e2.code} "
                               f"(intuit_tid={tid2}): {e2.read().decode()[:500]}") from e2
        _log.error("QBO %s %s failed: HTTP %s intuit_tid=%s", method, path, e.code, tid)
        raise QBOError(f"QBO {method} {path} failed: HTTP {e.code} "
                       f"(intuit_tid={tid}): {e.read().decode()[:500]}") from e
...
def fetch_company_info(cfg: QBOConfig, creds_path: Path) -> dict:
    """Read the connected company's profile — proves Bookie is on the real books."""
    resp = _api_call(cfg, creds_path, "GET", f"/companyinfo/{cfg.realm_id}")
    ci = resp.get("CompanyInfo", {})
    return {
        "company_name": ci.get("CompanyName"),
        "legal_name": ci.get("LegalName"),
        "country": ci.get("Country"),
        "started": ci.get("CompanyStartDate"),
...
def run_report(cfg: QBOConfig, creds_path: Path, name: str, params: dict | None = None) -> dict:
    """Generic QBO report fetch (e.g. ProfitAndLoss, BalanceSheet, GeneralLedger)."""
    return _api_call(cfg, creds_path, "GET", f"/reports/{name}", params=params or {})


def _count_report_data_rows(rows: dict | None) -> int:
    """Count transaction (Data) rows in a QBO report's nested Rows structure."""
    n = 0
    for r in (rows or {}).get("Row", []):
        if r.get("type") == "Data" and r.get("ColData"):
            n += 1
        if r.get("Rows"):
            n += _count_report_data_rows(r["Rows"])
    return n


def count_uncategorized(cfg: QBOConfig, creds_path: Path,
                        start_date: str = "2010-01-01",
                        end_date: str = "2026-12-31") -> dict:
    """Count transactions sitting in uncategorized buckets (Uncategorized * /
    Ask My Accountant), per account, via the GeneralLedger report.

    The entity query can't filter on line-level account, so we use the report —
    its `account` filter is honored (unlike TransactionList's, which is ignored).
    """
    accts = fetch_chart_of_accounts(cfg, creds_path)
    buckets = [a for a in accts
               if any(x in (a.get("name") or "").lower()
                      for x in ("uncategorized", "ask my accountant"))]
    per_account = {}
    total = 0
    for a in buckets:
        rep = run_report(cfg, creds_path, "GeneralLedger",
                         {"start_date": start_date, "end_date": end_date,
                          "account": a["id"], "columns": "tx_date,txn_type,name,subt_nat_amount"})
        c = _count_report_data_rows(rep.get("Rows", {}))
        per_account[a["name"]] = c
        total += c
    return {"accounts": [a["name"] for a in buckets], "per_account": per_account, "total": total}


def fetch_uncategorized_transactions(cfg: QBOConfig, creds_path: Path,
                                     start_date: str = "2010-01-01",
                                     end_date: str = "2026-12-31") -> list[dict]:
    """Pull the detail of every transaction sitting in an uncategorized bucket,
    via GeneralLedger. Returns {date, type, name, memo, amount, bucket} dicts."""
    accts = fetch_chart_of_accounts(cfg, creds_path)
    buckets = [a for a in accts
               if any(x in (a.get("name") or "").lower()
                      for x in ("uncategorized", "ask my accountant"))]
    ...
    for a in buckets:
        rep = run_report(cfg, creds_path, "GeneralLedger",
                         {"start_date": start_date, "end_date": end_date,
                          "account": a["id"], "columns": "tx_date,txn_type,name,memo,subt_nat_amount"})

        def walk(rows):
            for r in (rows or {}).get("Row", []):
                cd = r.get("ColData")
                if r.get("type") == "Data" and cd:
                    out.append({
                        "date": cd[0].get("value", "") if len(cd) > 0 else "",
                        "type": cd[1].get("value", "") if len(cd) > 1 else "",
                        "name": cd[2].get("value", "") if len(cd) > 2 else "",
                        "memo": cd[3].get("value", "") if len(cd) > 3 else "",
                        "amount": cd[-1].get("value", "") if cd else "",
                        "bucket": a["name"],
                    })
                if r.get("Rows"):
                    walk(r["Rows"])
